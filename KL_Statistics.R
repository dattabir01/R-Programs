# Defining a function for KL_Statistics

KL_Stats <- function(x, y){
  # Taking only the Relevant columns
                    ...
                    df$KLi = df$Pb * log(df$Pb/df$Pg)
                    KL = sum(df$KLi)
                    
                    print("Kullback - Liebler Statistics:");KL
                    return(KL)
}

# X = as.data.frame(data$goodbad)
# Y = as.data.frame(data$score)
# KL_Stats(X, Y)
